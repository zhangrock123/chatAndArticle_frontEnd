...
var swiper=(function () {
  var Swiper = function(element, options) {
    ...
    this.onInit = this.options.onInit || function() {}; 
    ...
    if(this.loop){
      ...
      if(this_eles.length>=2){
        this.element.innerHTML = ['<!--before-->','<!--center-->','<!--after-->','<!-- -->'].join(this.element.innerHTML.toString() );
  ...
  Swiper.prototype = {
    ...
    kill: function() {
      ...
      var removeLen= this.slides.length/3;
      var domList=[];
      while(pos--) {
        if(pos>removeLen && pos<removeLen*2){
          domList.push(this.slides[pos])
        }else{
          this.slides[pos].removeAttribute('style');
        }
      }
      for(var i =0,len=domList.length;i<len;i++){
        this.element.removeChild(domList[i]);
      }
      
      try{
        this.element.removeEventListener('touchstart', events, false);
        this.element.removeEventListener('webkitTransitionEnd', events, false);
        this.element.removeEventListener('msTransitionEnd', events, false);
        this.element.removeEventListener('oTransitionEnd', events, false);
        this.element.removeEventListener('otransitionend', events, false);
        this.element.removeEventListener('transitionend', events, false);
        window.removeEventListener('resize', events, false);
      }catch(e){

      }
      document.querySelector(this.indicate).innerHTML='';
    }
